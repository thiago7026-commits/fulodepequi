...
const priceSaveButton = document.getElementById("priceSave");
...
function loadSettings_() {
  ...
  if (!stored) return { airbnbLink: "", floatLink: "", dailyRate: DEFAULT_DAILY_RATE };
  try {
    const parsed = JSON.parse(stored);
    return {
      airbnbLink: typeof parsed.airbnbLink === "string" ? parsed.airbnbLink : "",
      floatLink: typeof parsed.floatLink === "string" ? parsed.floatLink : "",
      dailyRate: Number.isFinite(parsed.dailyRate) ? parsed.dailyRate : DEFAULT_DAILY_RATE,
    };
  } catch {
    return { airbnbLink: "", floatLink: "", dailyRate: DEFAULT_DAILY_RATE };
  }
}
...
function setPriceFeedback_(msg, color) {
  if (!priceFeedback) return;
  priceFeedback.textContent = msg;
  priceFeedback.style.color = color;
}
...
if (dailyRateInput) {
  const settings = loadSettings_();
  dailyRateInput.value = String(settings.dailyRate ?? DEFAULT_DAILY_RATE);
}

if (priceSaveButton && dailyRateInput) {
  priceSaveButton.addEventListener("click", () => {
    const settings = loadSettings_();
    const parsed = Number(dailyRateInput.value);
    if (!Number.isFinite(parsed) || parsed <= 0) {
      setPriceFeedback_("Informe um valor válido.", "#b34b39");
      return;
    }
    saveSettings_({
      airbnbLink: settings.airbnbLink,
    ...
    setPriceFeedback_("Valor salvo no navegador.", "#2f3b2a");
  });
}
